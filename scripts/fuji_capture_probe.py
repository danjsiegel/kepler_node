...
def restart_profile(runner: Runner, profile_name: str) -> None:
    ...
    script = (
        "curl -sf -X POST http://127.0.0.1:8624/api/server/stop >/dev/null && "
        f"curl -sf -X POST http://127.0.0.1:8624/api/server/start/{encoded} >/dev/null"
    )
    require_ok(runner.shell(script, timeout=15), "restart indiwebmanager profile")
...
def list_probe_files(runner: Runner, upload_dir: str, prefix: str) -> list[str]:
    pattern = shlex.quote(f"{upload_dir}/{prefix}*")
    result = runner.shell(f"ls -1 {pattern} 2>/dev/null || true", timeout=10)
    return [line.strip() for line in result.stdout.splitlines() if line.strip()]


def clear_probe_files(runner: Runner, upload_dir: str, prefix: str) -> None:
    runner.shell(f"rm -f {shlex.quote(upload_dir + '/' + prefix)}*", timeout=10)
...
def main() -> int:
    ...
    if connection != "On":
        fail(f"camera is not connected in INDI (CONNECTION.CONNECT={connection or 'missing'})")
